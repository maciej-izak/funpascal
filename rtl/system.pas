...
function GetStdHandle(nStdHandle: Integer): LongInt; external 'KERNEL32.DLL' name 'GetStdHandle';

procedure SetConsoleMode(hConsoleHandle: LongInt; 
                         dwMode: LongInt); external 'KERNEL32.DLL' name 'SetConsoleMode';

function CreateFileA(var lpFileName: string; 
                     dwDesiredAccess: LongInt;
                     dwShareMode: LongInt;
                     lpSecurityAttributes: Pointer; 
                     dwCreationDisposition, 
                     dwFlagsAndAttributes, 
                     hTemplateFile: LongInt): LongInt; external 'KERNEL32.DLL' name 'CreateFileA';
...
procedure CloseHandle(hObject: LongInt); external 'KERNEL32.DLL' name 'CloseHandle';
...
procedure IStr(Number: Integer; var s: string); forward;
procedure Str(Number: Real; var s: string; DecPlaces: Integer); forward;
...
procedure WriteCharF(var F: file; P: PStream; c: char; MinWidth, DecPlaces: Integer);
var
  i: Integer;
begin
for i := 1 to MinWidth - 1 do
  WriteCh(F, P, ' ');
WriteCh(F, P, c);
end;
...
const
  FracBits = 16;
var
  Integ, Frac, InvWeight, FinalInvWeight, Digit, IntegExpon, i: Integer;
  Expon: Real;
...
if ((Expon > 8) or (Expon < -3)) and (DecPlaces <= 0) then
  begin
...
for i := 1 to DecPlaces do
  FinalInvWeight := FinalInvWeight * 10;

while InvWeight <= FinalInvWeight do
  begin
